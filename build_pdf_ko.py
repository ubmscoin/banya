#!/usr/bin/env python3
"""
Banya Framework — Merge all Korean HTML sections into one PDF-ready HTML,
...
def extract_lang_en(html, filename):
    """Extract all <div class="lang-ko" ...>...</div> blocks."""
    # Find all lang-ko divs (they can be nested, so we do balanced brace matching)
    blocks = []
    pattern = r'<div\s+class="lang-ko"[^>]*>'
    for m in re.finditer(pattern, html):
        ...
        while i < len(html):
            ...
            i += 1
    if not blocks:
        print(f"  WARNING: No lang-ko content found in {filename}")
    return "\n".join(blocks)

def extract_en_footer(html):
    """Extract English footer if exists."""
    m = re.search(r'<div\s+class="doc-footer\s+lang-ko"[^>]*>(.*?)</div>\s*(?=<div|<script|$)', html, re.DOTALL)
    if m:
        return m.group(0)
    return ""
...
def build_combined_html():
    ...
    .PLACEHOLDER_BLOCK !important; }
    .lang-switch, .toc-side, .toc-bar, .toc-overlay, .page-nav { display: none !important; }
    .doc-footer.lang-en { display: none !important; }
    ...
    combined = "".join(parts)
    outpath = os.path.join(BASE, "banya_combined_ko.html")
