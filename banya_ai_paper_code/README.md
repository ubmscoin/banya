...
The three steps below reproduce every measurement in the six papers. The requirements are an NVIDIA GPU (CUDA) and Python 3.12; install the Python packages with the single line below. For cupy, choose the build that matches your CUDA version (for example, cupy-cuda13x for CUDA 13).
...
To download by hand, fetch bitok_elem2_170000_m.npz, cache_elem3_190000.npz, and world_toddler2_110000_m.npz from the record page above, place them in the model folder, and verify with `sha256sum -c checksums.sha256`. Details are in [model/DOWNLOAD.md](model/DOWNLOAD.md).
...
The eleven probes in the probe folder reproduce the measured figures of Papers 1 through 6. The header of each file states what it measures and carries its run line. For example, the verification of Paper 1 runs as follows.
...
- **[model/](model/)** — Frozen checkpoints and evaluation data. The large npz files come from Zenodo (doi.org/10.5281/zenodo.21383724): bash download.sh
  ...
  - [download.sh](model/download.sh) — Downloads the three checkpoints at once and verifies integrity: bash download.sh
  - [checksums.sha256](model/checksums.sha256) — Expected checkpoint integrity values
...
- **[probe/](probe/)** — Eleven measurement probes. Each file header carries its run line; execute with python3 filename
...
아래 세 단계면 논문 여섯 편의 모든 실측이 재현된다. 요구 환경은 NVIDIA GPU(CUDA)와 Python 3.12 이며, 파이썬 패키지는 다음 한 줄로 설치한다. cupy 는 설치된 CUDA 버전에 맞는 배포판을 고른다(예: CUDA 13 은 cupy-cuda13x).
...
손으로 받으려면 위 기록 페이지에서 bitok_elem2_170000_m.npz, cache_elem3_190000.npz, world_toddler2_110000_m.npz 세 파일을 받아 model 폴더에 넣고 `sha256sum -c checksums.sha256` 으로 검증한다. 자세한 안내는 [model/DOWNLOAD.md](model/DOWNLOAD.md) 에 있다.
...
probe 폴더의 프로브 11개가 논문 1편부터 6편까지의 실측 수치를 재생한다. 각 파일의 머리말에 무엇을 측정하는지와 실행 줄이 적혀 있다. 예를 들어 제1편의 검증은 다음과 같다.
...
- **[model/](model/)** — 얼린 체크포인트와 평가 자료. 대용량 npz 는 제노도(doi.org/10.5281/zenodo.21383724)에서 받는다. bash download.sh
  ...
  - [download.sh](model/download.sh) — 체크포인트 3개를 한 번에 받고 무결성을 검증. bash download.sh
  - [checksums.sha256](model/checksums.sha256) — 체크포인트 무결성 기대값
...
- **[probe/](probe/)** — 논문별 실측 프로브 11종. 각 파일 머리말에 실행 줄이 있고 python3 파일명 으로 실행한다
